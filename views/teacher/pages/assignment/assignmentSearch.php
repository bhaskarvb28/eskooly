...
// Count total
$stmt = $conn->prepare("SELECT COUNT(*) as total FROM contents WHERE content_title LIKE :searchTerm AND content_type = 'assignment'");
$stmt->execute(['searchTerm' => $searchTerm]);
$totalResults = (int) $stmt->fetch(PDO::FETCH_ASSOC)['total'];
$totalPages = max(1, ceil($totalResults / $resultsPerPage));

// Fetch paginated results
$stmt = $conn->prepare("SELECT * FROM contents WHERE content_title LIKE :searchTerm AND content_type = 'assignment' LIMIT :limit OFFSET :offset");

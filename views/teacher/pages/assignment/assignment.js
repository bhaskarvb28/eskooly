function initAssignmentPage() {
  ...
  function fetchAssignment(query = "", page = 1) {
    fetch(
      `pages/assignment/assignmentSearch.php?query=${encodeURIComponent(
        query
      )}&page=${page}`
    )
      .then((res) => res.json())
  ...
  function renderTable(rows) {
    ...
    if (rows.length == 0 && searchInput.value == "") {
      tableBody.innerHTML = `<tr><td colspan="10">No assignment Uploaded yet</td></tr>`;
  ...
  function renderPagination(current, total) {
    ...
    if (current > 1) {
      prev.href = "#";
      prev.onclick = () => fetchAssignment(currentQuery, current - 1);
    } else {
    ...
    if (current < total) {
      next.href = "#";
      next.onclick = () => fetchAssignment(currentQuery, current + 1);
    } else {

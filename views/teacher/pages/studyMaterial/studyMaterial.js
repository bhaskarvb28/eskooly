function initStudyMaterialPage() {
  ...
  function fetchStudyMaterial(query = "", page = 1) {
    fetch(
      `pages/studyMaterial/studyMaterialSearch.php?query=${encodeURIComponent(
        query
      )}&page=${page}`
    )
      .then((res) => res.json())
  ...
  function renderTable(rows) {
    ...
    if (rows.length == 0 && searchInput.value == "") {
      tableBody.innerHTML = `<tr><td colspan="10">No Study Material Uploaded yet</td></tr>`;
  ...
  function renderPagination(current, total) {
    ...
    if (current > 1) {
      prev.href = "#";
      prev.onclick = () => fetchStudyMaterial(currentQuery, current - 1);
    } else {
    ...
    if (current < total) {
      next.href = "#";
      next.onclick = () => fetchStudyMaterial(currentQuery, current + 1);
    } else {

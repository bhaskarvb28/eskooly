function initOtherDownloadsPage() {
  ...
  function fetchOtherDownloads(query = "", page = 1) {
    fetch(
      `pages/otherDownloads/otherDownloadsSearch.php?query=${encodeURIComponent(
        query
      )}&page=${page}`
    )
      .then((res) => res.json())
  ...
  function renderTable(rows) {
    ...
    if (rows.length == 0 && searchInput.value == "") {
      tableBody.innerHTML = `<tr><td colspan="10">No Downloads Uploaded yet</td></tr>`;

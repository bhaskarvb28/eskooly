...
// Count total
$stmt = $conn->prepare("SELECT COUNT(*) as total FROM book_categories WHERE name LIKE :searchTerm");
$stmt->execute(['searchTerm' => $searchTerm]);
$totalResults = (int) $stmt->fetch(PDO::FETCH_ASSOC)['total'];
$totalPages = max(1, ceil($totalResults / $resultsPerPage));

// IMPORTANT FIX HERE:
$sql = "SELECT * FROM book_categories WHERE name LIKE :searchTerm LIMIT $resultsPerPage OFFSET $offset";
